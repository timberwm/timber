...
static struct tmbr_xdg_client *tmbr_server_find_focus(struct tmbr_server *server)
{
	...
	if (!server->focussed_output)
		return NULL;
	return server->focussed_output->focus->focus;
}
...
static void tmbr_xdg_popup_on_destroy(struct wl_listener *listener, TMBR_UNUSED void *payload)
{
	...
	tmbr_unregister(&popup->base.destroy);
	tmbr_unregister(&popup->base.commit);
	tmbr_unregister(&popup->base.new_popup);
	free(popup);
}
...
static void tmbr_xdg_client_on_destroy(struct wl_listener *listener, TMBR_UNUSED void *payload)
{
	...
	tmbr_unregister(&client->base.destroy);
	tmbr_unregister(&client->base.commit);
	tmbr_unregister(&client->base.map);
	tmbr_unregister(&client->base.unmap);
...
static void tmbr_output_on_destroy(struct wl_listener *listener, TMBR_UNUSED void *payload)
{
	...
	tmbr_unregister(&output->destroy);
	tmbr_unregister(&output->frame);
	tmbr_unregister(&output->commit);
	tmbr_unregister(&output->request_state);
...
static void tmbr_keyboard_on_destroy(struct wl_listener *listener, TMBR_UNUSED void *payload)
{
	...
	tmbr_unregister(&keyboard->destroy);
	tmbr_unregister(&keyboard->key);
	tmbr_unregister(&keyboard->modifiers);
	free(keyboard);
}
...
static void tmbr_layer_client_on_new_popup(struct wl_listener *listener, void *payload)
{
	struct tmbr_layer_client *layer_client = wl_container_of(listener, layer_client, base.new_popup);
	int x, y;

	wlr_scene_node_coords(&layer_client->scene_layer_surface->tree->node, &x, &y);
	tmbr_xdg_popup_new(payload, layer_client->scene_layer_surface->tree, (struct wlr_box){
				    ...
				    void *payload TMBR_UNUSED)
{
	...
	tmbr_unregister(&pointer->destroy);
	free(pointer);
}
...
static void tmbr_server_on_new_xdg_toplevel(struct wl_listener *listener, void *payload)
{
	...
	tmbr_register(&toplevel->base->events.new_popup, &client->base.new_popup, tmbr_xdg_client_on_new_popup);
...
static void tmbr_server_on_new_layer_shell_surface(struct wl_listener *listener, void *payload)
{
	...
	tmbr_register(&surface->events.new_popup, &client->base.new_popup, tmbr_layer_client_on_new_popup);
}

static void tmbr_server_on_new_session_lock_surface(struct wl_listener *listener, TMBR_UNUSED void *payload)
{
	struct tmbr_server *server = wl_container_of(listener, server, new_session_lock_surface);
	struct wlr_session_lock_surface_v1 *lock_surface = payload;
	struct tmbr_output *output = lock_surface->output->data;
	struct wlr_scene_tree *scene_tree;

	scene_tree = wlr_scene_subsurface_tree_create(output->scene_layers[TMBR_SCENE_LAYER_LOCK], lock_surface->surface);
...
static void tmbr_server_on_session_lock_unlock(struct wl_listener *listener, TMBR_UNUSED void *payload)
{
	struct tmbr_server *server = wl_container_of(listener, server, session_lock_unlock);
	struct tmbr_xdg_client *focus;
	server->locked = false;
	if ((focus = tmbr_server_find_focus(server)) != NULL)
		tmbr_xdg_client_focus(focus, true);
	tmbr_unregister(&server->new_session_lock_surface);
	tmbr_unregister(&server->session_lock_unlock);
}

static void tmbr_server_on_new_session_lock(struct wl_listener *listener, void *payload)
{
	struct tmbr_server *server = wl_container_of(listener, server, new_session_lock);
	struct wlr_session_lock_v1 *lock = payload;
	...
	tmbr_register(&lock->events.new_surface, &server->new_session_lock_surface, tmbr_server_on_new_session_lock_surface);
	tmbr_register(&lock->events.unlock, &server->session_lock_unlock, tmbr_server_on_session_lock_unlock);
...
static void tmbr_server_on_destroy_drag_icon(TMBR_UNUSED struct wl_listener *listener, void *payload)
{
	...
	tmbr_unregister(&server->destroy_drag_icon);
}
...
static void tmbr_idle_inhibitor_on_destroy(struct wl_listener *listener, TMBR_UNUSED void *payload)
{
	...
	tmbr_unregister(&inhibitor->on_destroy);
}
